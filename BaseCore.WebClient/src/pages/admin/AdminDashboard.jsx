...
export default function AdminDashboard({
  ...
  // ── KPI tài chính (luôn hiển thị, cả admin lẫn operator)
  const financeCards = [
    ["Tổng doanh thu",      formatVND(pick(summary, ["totalRevenue",  "TotalRevenue",  "revenue", "Revenue"], 0)), "fa-money-bill-wave", "#ea580c"],
    ["Doanh thu hôm nay",   formatVND(pick(summary, ["todayRevenue",  "TodayRevenue"],  0)),                       "fa-calendar-day",   "#16a34a"],
    ["Doanh thu tháng này", formatVND(pick(summary, ["monthRevenue",  "MonthRevenue"],  0)),                       "fa-calendar-check", "#0ea5e9"],
  ...
  return (
    <>
      ...
      {/* ── Zone 1: KPI tài chính ── */}
      <section className="admin-stats dashboard-stats dashboard-stats--finance">
        {financeCards.map(([label, value, icon, color]) => (
          <div
            className="stat-card"
            key={label}
      ...
      {/* ── Zone 2: KPI vận hành + admin-only ── */}
      <section className="admin-stats dashboard-stats dashboard-stats--ops">
        {operationCards.map(([label, value, icon, color]) => (
          <div
            className="stat-card"
            key={label}
            style={{ borderLeft: `4px solid ${color}` }}
        ...
        {!isOperator &&
          adminOnlyCards.map(([label, value, icon, color]) => (
            <div
              className="stat-card"
              key={label}
              style={{ borderLeft: `4px solid ${color}` }}
      ...
      {/* ── Zone 3: Chart doanh thu + Trạng thái ── */}
      <section className="dashboard-chart-grid">
        <div className="admin-card">
          <h3>Doanh thu theo ngày</h3>
          <MiniBarChart
        ...
        <div className="admin-card">
          ...
          <MiniBarChart
            items={dashboard.revenueByMonth}
            labelFor={(item) =>
              `${pick(item, ["month", "Month"])}/${pick(item, ["year", "Year"])}`
            }
          />
        </div>
      </section>

      {/* ── Zone 4: Top tuyến + Top nhà xe (admin only) ── */}
      <section className="dashboard-chart-grid">
        <RankTable
          title="Top tuyến bán chạy"
          items={dashboard.topRoutes}
        ...
        {!isOperator && (
          <RankTable
            ...
            items={dashboard.topOperators}
            nameFor={(item) =>
              pick(item, ["operatorName", "OperatorName"], "Chưa rõ nhà xe")
            }
          />
        )}
      </section>

      {/* ── Zone 5: Trạng thái booking + thanh toán ── */}
      <section className="dashboard-chart-grid">
        <div className="admin-card">
          <h3>Trạng thái booking</h3>
          <div className="dashboard-status-list">
            {bookingStatuses.map((item) => (
              <div key={pick(item, ["status", "Status"])}>
                <StatusBadge>
                  {labelBookingStatus(pick(item, ["status", "Status"]))}
                </StatusBadge>
                <strong>{pick(item, ["count", "Count"], 0)}</strong>
              </div>
            ))}
          ...
          <div className="admin-card">
            ...
            <div className="dashboard-status-list">
              {paymentStatuses.map((item) => (
                <div key={pick(item, ["status", "Status"])}>
                  <StatusBadge>
                    {labelPaymentStatus(pick(item, ["status", "Status"]))}
                  </StatusBadge>
                  <strong>{pick(item, ["count", "Count"], 0)}</strong>
                </div>
              ))}
      ...
      {/* ── Zone 6: Chuyến sắp chạy + Đơn mới nhất ── */}
      <section className="admin-grid">
        <div className="admin-card">
          <h3>Chuyến sắp chạy</h3>
          <DashboardTripsTable trips={upcomingTrips.slice(0, 5)} />
                ...
                {bookings.slice(0, 6).map((booking) => {
                  ...
                  return (
                    <tr key={id}>
                      ...
                      <td>
                        {formatVND(pick(booking, ["totalPrice", "TotalPrice"], 0))}
                      </td>
                    </tr>
                  );
                })}
